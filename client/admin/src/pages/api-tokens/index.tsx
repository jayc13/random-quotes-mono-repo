  ...
  const [create, { isLoading }] = useCreate<NewApiToken>(
    ...
    { name: tokenName }, // The data to send for creation
    {
      onSuccess: (data) => {
        ...
        notify(
          "API Token created successfully. Copy the token now, it will not be shown again.",
          { type: "success" },
        );
      ...
      onError: (error) => {
        notify(`Error creating token: ${error.message || "Unknown error"}`, {
          type: "warning",
        });
    ...
    <Dialog open={open} onClose={onClose} maxWidth='md'>
      ...
      <DialogContent>
        <DialogContentText color='error' sx={{ fontWeight: "bold", mb: 2 }}>
          Warning: This is the only time your token will be displayed. Copy it
          now and store it securely. You will not be able to see it again.
        </DialogContentText>
        <Box
          sx={{
            p: 2,
...
export const ApiTokensList = () => {
  ...
  const [newlyCreatedToken, setNewlyCreatedToken] =
    useState<NewApiToken | null>(null);
  ...
  const { data, isLoading, error } = useList<ApiTokenInfo>({
    resource: "api-tokens",
  });
  ...
  if (error) {
    return (
      <Typography color='error'>
        Error loading API tokens: {error.message}
      </Typography>
    );
  }
