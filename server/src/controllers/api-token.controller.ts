...
export const createApiTokenHandler = async (
  ...
  if (!userId) {
    return Response.json({ error: 'Unauthorized', message: 'User ID not found in context.' }, { status: 401, headers: DEFAULT_CORS_HEADERS });
  }
  ...
  try {
    ...
    if (!input || !input.name || typeof input.name !== 'string' || input.name.trim() === '') {
        throw new Error('Invalid input: Token name is required and must be a non-empty string.');
    }
  } catch (e) {
    const message = e instanceof Error ? e.message : 'Invalid JSON body.';
    return Response.json({ error: 'Bad Request', message }, { status: 400, headers: DEFAULT_CORS_HEADERS });
  }
  ...
  try {
    ...
    return Response.json({ error: 'Internal Server Error', message }, { status: 500, headers: DEFAULT_CORS_HEADERS });
  }
};
...
export const getUserApiTokensHandler = async (
  ...
  try {
    ...
    return Response.json({ error: 'Internal Server Error', message: 'Failed to retrieve API tokens.' }, { status: 500, headers: DEFAULT_CORS_HEADERS });
  }
};
...
export const deleteApiTokenHandler = async (
  ...
  if (isNaN(tokenId) || tokenId <= 0) {
      return Response.json({ error: 'Bad Request', message: 'Invalid Token ID provided.' }, { status: 400, headers: DEFAULT_CORS_HEADERS });
  }
  ...
  try {
    ...
    if (!deleted) {
      ...
      return Response.json({ error: 'Not Found', message: 'API Token not found or you do not have permission to delete it.' }, { status: 404, headers: DEFAULT_CORS_HEADERS });
    }

    // Successfully deleted
    // Return the deleted record ID, useful for some frontend frameworks like react-admin
    return Response.json({ id: tokenId } , {
    ...
    return Response.json({ error: 'Internal Server Error', message: 'Failed to delete API token.' }, { status: 500, headers: DEFAULT_CORS_HEADERS });
  }
};
